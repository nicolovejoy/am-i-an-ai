...
export class MatchManager {
    ...
    if (participant?.type === 'human') {
      ...
      for (const aiParticipant of aiParticipants) {
        if (!currentRound.responses[aiParticipant.identity]) {
          const robotResponse = this.generateRobotResponse(
            aiParticipant.identity,
            currentRound.prompt,
            (aiParticipant.personality as AIPersonality) || 'curious_student'
          );
   ...
   */
  generateRobotResponse(_identity: Identity, _prompt: string, personality: AIPersonality): string {
