...
export interface MatchEventSequence {
  matchId: string;
  events: MatchEvent[];
}
 ...
 */
export async function generateCompleteMatch(matchId: string): Promise<MatchEvent[]> {
  ...
  const matchStartedData: MatchStartedData = {
    participants,
    humanParticipant,
    robotParticipants,
  ...
  // 2. Generate 5 rounds
  const allResponses: Array<{ participantId: string; response: string }> = [];
  ...
  for (let round = 1; round <= 5; round++) {
    ...
    const roundResponses: Array<{ participantId: string; response: string }> = [];

    for (const participantId of responseOrder) {
      let responseData: ResponseSubmittedData;
      ...
      if (participantId === humanParticipant) {
        ...
        events.push(createMatchEvent(
          matchId,
          EVENT_TYPES.RESPONSE_SUBMITTED,
          responseData,
          currentTimestamp
        ));
      } else {
    ...
    // Round Completed
    const roundCompletedData: RoundCompletedData = {
      round,
  ...
  const votingStartedData: VotingStartedData = {
    allResponses,
  ...
  const voteSubmittedData: VoteSubmittedData = {
 ...
 */
export async function createSampleMatches(count: number): Promise<MatchEventSequence[]> {
  const matches: MatchEventSequence[] = [];
 ...
 */
function generateHumanResponse(_prompt: string, _round: number): string {
 ...
 */
function generateRobotResponse(_prompt: string, robotType: string, _round: number): string {
  const personality = ROBOT_PERSONALITIES[robotType as keyof typeof ROBOT_PERSONALITIES];
  ...
  const responsePool = responses[robotType as keyof typeof responses] || responses['curious-student'];
  return responsePool[Math.floor(Math.random() * responsePool.length)];
}

// Helper functions for human response generation
function getRandomExperience(_prompt: string): string {
  const experiences = [
...
function getRandomSignificance(): string {
  const significances = [
...
function getRandomResult(): string {
  const results = [
...
function getRandomObservation(): string {
  const observations = [
 ...
 */
export function exportEventsForKafka(events: MatchEvent[]): Array<{
  topic: string;
  key: string;
  value: string;
  timestamp: number;
